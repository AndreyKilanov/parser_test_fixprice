# Парсер товаров сайта fix-price.com (тестовое задание)
...
```
ИИспользуя фреймворк Scrapy необходимо написать парсер для получения информации о товарах интернет-магазина из списка категорий по заранее заданному шаблону, 
...
```bash
git clone git@github.com:AndreyKilanov/parser_test_fixprice.git
```
...
```
Результаты будут находиться `/src/data/fixprice.json`
